...
<xsl:output encoding="UTF-8" indent="yes" method="html"/>
	<xsl:template match="/">
				<CENTER><SPAN class="ymPageHead">Trip Listings - <xsl:value-of select="/listing/@month" /><xsl:text disable-output-escaping="yes">  </xsl:text><xsl:value-of select="/listing/@year" /></SPAN></CENTER>
				<P/>
				<TABLE><tbody class="ymText">
				<xsl:for-each select="listing/trips">
					<TR><TD COLSPAN="2"><A href="#{@link}"><xsl:value-of select="@id" /></A></TD></TR>	
				</xsl:for-each>
				</tbody></TABLE>
				<p/>
				...
				<BR/>
				<SPAN class="ymText"><xsl:value-of disable-output-escaping="yes" select="listing/printheader" /></SPAN>
				<P/>
